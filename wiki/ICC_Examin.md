...
In normal mode it shows first the tag list with the tag viewer below.
This layout is much like the familiar ColorSync profile viewer layout.
Above the tag list resides the applications menubar. From there you can
select File-&gt;Open and the file dialog appears. Select one or more
files for gamut comparision as you like. On top right side of the FLTK
file selector resides the bookmark button. If you frequently visit
directories you will find it usefull to append these directories to yout
bookmark or favourites list. To do this, go to an directory with the
file selector and choose from the bookmarkbutton the add entry. The
added directory appears in the bookmarklist and is available for later
use.
...
-   middle mouse button or Shift+middle mouse button to move/clip
...
select the line of average human (CIE standard observer) visible
spectral colours as a line.

If meashurements are shown, theyr radius can be selected from the
...
ncl2 colours can be enlarged or made smaller bypressing + or -.
...
data are compared to what is possible to calculate with the prifile and
littleCMS - the internal used CMM.
...
line. Each like contains the the colour values and euclidian dE +
...
The report can be exported from the File menu.

#### CGATS view

The Measurement data itself are parsed in the profile and transformed to
fit in the CGATS standard. A window can be show from the View menu to
show the corrected measurement output.
